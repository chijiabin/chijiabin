...
+(instancetype)MenuViewWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    YMenuView *menu = [[YMenuView alloc]initWithDependencyView:dependencyView MenuView:leftmenuView isShowCoverView:isCover];
    return menu;
}
...
-(instancetype)initWithDependencyView:(UIView *)dependencyView MenuView:(UIView *)leftmenuView isShowCoverView:(BOOL)isCover{
    ...
    if(self = [super init]){
        ...
        self.coverViewframe = CGRectMake(self.menuViewframe.size.width, self.menuViewframe.origin.y, SCREEN_WIDTH - self.menuViewframe.size.width, self.menuViewframe.size.height);
    }
    return self;
}
 ...
 *  屏幕边缘pan手势(优先级高于其他手势)
 *  屏幕左侧边缘响应
 *  @param dependencyView <#dependencyView description#>
 */
...
#pragma make 显示左边=点击
    ...
    self.coverView.frame = CGRectMake(0, 0, SCREEN_WIDTH, self.menuViewframe.size.height);
    ...
    if(UIGestureRecognizerStateBegan == gesture.state ||
       UIGestureRecognizerStateChanged == gesture.state){
        if(translation.x <= self.menuViewframe.size.width){// && self.leftMenuView.frame.origin.x != 0
            if(translation.x <= 10){
                self.coverView.frame = CGRectMake(0, self.menuViewframe.origin.y, SCREEN_WIDTH, self.menuViewframe.size.height);
            }
            ...
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width+x, 0,SCREEN_WIDTH-self.leftMenuView.frame.size.width-x, self.menuViewframe.size.height);
    ...
    if(UIGestureRecognizerStateBegan == recognizer.state ||
       UIGestureRecognizerStateChanged == recognizer.state){
        ...
        if(Place <= self.leftMenuView.frame.size.width &&  Place >0){
            ...
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width-Place, 0,SCREEN_WIDTH-self.leftMenuView.frame.size.width+Place, self.menuViewframe.size.height);
            ...
            self.coverView.frame    = CGRectMake(self.leftMenuView.frame.size.width, 0,SCREEN_WIDTH-self.leftMenuView.frame.size.width, self.menuViewframe.size.height);
            self.coverView.alpha    = CoverViewAlpha;
        }
...
-(void)removeCoverAndMenuView{
    self.leftMenuView.frame = CGRectMake(-self.leftMenuView.frame.size.width, 0, self.leftMenuView.frame.size.width, SCREEN_HEIGHT);
    self.coverView.frame    = CGRectMake(0, 0,SCREEN_WIDTH, SCREEN_HEIGHT);
    self.coverView.alpha    = 0.0;
...
-(CGRect)menuViewframe{
    if((CGRectEqualToRect(_menuViewframe,CGRectZero)) == YES){
        //设置左侧菜单
        CGFloat w                   = SCREEN_WIDTH * MenuView_scale_of_Screen;
...
-(CGRect)coverViewframe{
    ...
    if((CGRectEqualToRect(_coverViewframe, CGRectZero)) == YES){
        _coverViewframe = CGRectMake(self.menuViewframe.size.width, self.menuViewframe.origin.y, SCREEN_WIDTH - self.menuViewframe.size.width, self.menuViewframe.size.height);
    }
