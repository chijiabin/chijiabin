...
typedef NS_ENUM(NSInteger, SearchConfigType)  {
    EnterText,
    OnclickSearch
};


typedef void(^PYSearchSuggestionDidSelectCellBlock)(NSString *searchContent ,NSString *Cid);

@interface PYSearchSuggestionViewController : UITableViewController

@property (nonatomic ,assign) SearchConfigType searchType;
