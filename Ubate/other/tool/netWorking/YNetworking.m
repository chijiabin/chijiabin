...
#define QC_ERROR [NSError errorWithDomain:@"com.Joe78.YNetworking.ErrorDomain" code:-999 userInfo:@{ NSLocalizedDescriptionKey:QC_ERROR_IMFORMATION}]
...
@interface YNetworking()
@property (nonatomic, copy) NSString *baseUrl;

@end

@implementation YNetworking
+ (void)initialize{
    ...
    //检查网络
    [YNetworking checkNetworkStatus];
...
+ (NSURLSessionTask *)getRequestWithUrl:(NSString *)url
    ...
    if (isCache) {
        ...
        if (responseObject) {
            ...
            if (responseObject) {
                //这个字段取决于 服务器
                code                = [responseObject[@"res"] intValue];
...
//    [url stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]
    ...
    session = [_manager GET:[url stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet  URLQueryAllowedCharacterSet]] parameters:params progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
    ...
    [mgr setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        ...
        switch (status) {
            ...
            case AFNetworkReachabilityStatusReachableViaWWAN: // 手机自带网络
                ...
                [[NSNotificationCenter defaultCenter] postNotificationName:RECONNECTNETWORK object:nil];
                NSLog(@"手机自带网络");
                break;
...
+ (void)saveDownloadData:(NSData *)data
    ...
    if (!directoryPath) {
        directoryPath = [[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"YNetworking"] stringByAppendingPathComponent:@"download"];
